...
static int debug_render(pbWindow* w, Rectangle rect, int input) {
	...
	switch (active_tab) {
		case 0 : { // INFO TAB
			...
			y += SUS_HEIGHT - 10;
			GuiDrawText(pbSummaryGetInfo(), (Rectangle){rect.x, y, rect.width, 
				100}, TEXT_ALIGN_CENTER, GetColor(GuiGetStyle(DEFAULT, GuiGetState()
					== STATE_DISABLED ? BORDER_COLOR_DISABLED : LINE_COLOR)
				)
			);
			y += 100;
